...
const CHURCHES = {
  ...
  toulouse: { id:"toulouse", name:"Toulouse", fullName:"Église de Toulouse", color:"#059669", bg:"#ECFDF5",
            serviceDays:[6], repDays:[], dayLabel:"Sam" },
};
...
const NOTES_M= ["Dom","Do#m","Rém","Ré#m","Mim","Fam","Fa#m","Solm","Sol#m","Lam","La#m","Sim"];
...
.cswb.toulouse{background:var(--grn-bg);border-color:var(--grn);color:var(--grn);}
...
const FLYER_COLORS = {
  lognes: {
    ...
    bodyBg: "linear-gradient(180deg,#fce4f3 0%,#f5c8e8 100%)",
    sectionColor: "#b5106e",
    iconBg: "#e91e8c",
  },
  creil: {
    ...
    bodyBg: "linear-gradient(180deg,#ede8fd 0%,#ddd5fb 100%)",
    sectionColor: "#3d0fa3",
    iconBg: "#5b21d4",
  },
  toulouse: {
    headerGrad: "linear-gradient(135deg,#03543a 0%,#059669 60%,#4ade9d 100%)",
    card: "#0d9668",
    footer: "#03543a",
    badge: "rgba(255,255,255,.18)",
    bodyBg: "linear-gradient(180deg,#e2fbef 0%,#bdf5da 100%)",
    sectionColor: "#03543a",
    iconBg: "#059669",
  },
};
...
function FlyerModal({church,st,month,year,onClose}){
  ...
  function shareWhatsApp(){
    const lines=[
      ...
      ...(sched.atelier?[`📖 *${sched.atelier.title} (${sched.atelier.label}s)*`,
          ...
          <div style={{padding:"20px 22px 24px",background: fc.bodyBg}}>
            ...
            <div style={{marginBottom:20}}>
              <div style={{display:"flex",alignItems:"center",gap:8,fontFamily:"'Bebas Neue',sans-serif",fontSize:"1.2rem",letterSpacing:"2px",
                color: fc.sectionColor,marginBottom:10,paddingBottom:7,borderBottom:"2px solid rgba(0,0,0,.1)"}}>
                <div style={{width:28,height:28,borderRadius:7,background: fc.iconBg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"0.9rem",color:"white"}}>{sched.atelier.icon}</div>
            ...
            <div>
              <div style={{display:"flex",alignItems:"center",gap:8,fontFamily:"'Bebas Neue',sans-serif",fontSize:"1.2rem",letterSpacing:"2px",
                color: fc.sectionColor,marginBottom:10,paddingBottom:7,borderBottom:"2px solid rgba(0,0,0,.1)"}}>
                <div style={{width:28,height:28,borderRadius:7,background: fc.iconBg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"0.9rem",color:"white"}}>{sched.culte.icon}</div>
                {sched.culte.title} — {sched.culte.label}s
              </div>
              <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
...
export default function App() {
  ...
  const validate=(cid,mk)=>{
    ...
    upd(s=>{
      ...
      if(!s.validatedMonths)s.validatedMonths={creil:{},lognes:{},toulouse:{}};
...
function AccueilTab({user,isAdmin,st,verset,showNotifBanner,onDismissNotif,onGoDispos,month,year,prevMonth,nextMonth,church}){
  ...
  const churchMemberStats=Object.values(CHURCHES).map(c=>({n:(st.members[c.id]||[]).length,l:`Membres ${c.name}`,icon:"⛪",color:c.color,bg:c.bg}));
...
function MusicienTab({user,st,church}){
  ...
  // Clé de stockage scopée par programme + chant, pour que la tonalité choisie
  // pour un chant dans un programme ne "fuite" pas vers ce même chant dans un autre programme.
  function skey(sid){return `${activeProg?.id||"noprog"}_${sid}`;}
  function getKey(sid,orig){const k=songKeys[skey(sid)]||(orig||"Do");return k.replace(/^Re(?=#|b|$)/,"Ré");}
  function changeKey(sid,orig,delta){
    ...
    if(ci===-1)return;
    setSongKeys(k=>{const nk={...k,[skey(sid)]:CUR_NOTES[(ci+delta+12)%12]};try{localStorage.setItem("jclc_keys_"+user?.id,JSON.stringify(nk));}catch{}return nk;});
  }
      ...
      <div style={{position:"fixed",inset:0,background:"#0f172a",color:"#f1f5f9",zIndex:9999,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:"rgba(0,0,0,.6)",borderBottom:"1px solid rgba(255,255,255,.1)",flexShrink:0,flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",gap:2}}>
            {CUR_NOTES.map(n=><button key={n} onClick={()=>setSongKeys(k=>{const nk={...k,[skey(sid)]:n};try{localStorage.setItem("jclc_keys_"+user?.id,JSON.stringify(nk));}catch{}return nk;})} style={{padding:"2px 5px",borderRadius:4,fontSize:10,fontWeight:700,background:dispKey===n?"#6366f1":"rgba(255,255,255,.1)",color:"#fff",border:"none",cursor:"pointer"}}>{n}</button>)}
          </div>
          <button className="btn btn-g btn-xs" onClick={()=>changeKey(sid,origKey,+1)}>+1</button>
            ...
            return(
              <div key={sid} className="card" style={{margin:"0 16px 12px",borderLeft:`4px solid ${ch.color}`}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:3,flexWrap:"wrap",marginBottom:hasSections?8:0}}>
                  <button className="btn btn-g btn-xs" onClick={()=>changeKey(sid,origKey,-1)}>−1</button>
                  {CUR_NOTES.map(n=><button key={n} onClick={()=>setSongKeys(k=>{const nk={...k,[skey(sid)]:n};try{localStorage.setItem("jclc_keys_"+user?.id,JSON.stringify(nk));}catch{}return nk;})} className={`kbtn${dispKey===n?" on":""}`} style={{padding:"2px 6px",fontSize:10}}>{n}</button>)}
                  <button className="btn btn-g btn-xs" onClick={()=>changeKey(sid,origKey,+1)}>+1</button>
                  {dispKey!==origKey&&<button className="btn btn-g btn-xs" onClick={()=>setSongKeys(k=>{const nk={...k};delete nk[skey(sid)];try{localStorage.setItem("jclc_keys_"+user?.id,JSON.stringify(nk));}catch{}return nk;})}>↺ {origKey}</button>}
                </div>
                {hasSections&&(
                  <details>
...
function ProgramFormModal({songs,program,onSave,onClose,churchId}){
  ...
  return(
    <div className="modal" style={{width:620}}>
      ...
      <div className="fld" style={{marginBottom:12}}><label>Assemblée</label><select className="sel" value={progChurch} onChange={e=>setProgChurch(e.target.value)}>{Object.values(CHURCHES).map(c=><option key={c.id} value={c.id}>{c.fullName}</option>)}</select></div>
      <div className="g2">
      ...
      <div style={{maxHeight:300,overflowY:"auto"}}>
        {items.map((item,i)=>{const s=songs.find(x=>x.id===item.songId);return s?(
          <div key={i} style={{background:"var(--sur2)",borderRadius:"var(--r)",padding:"12px 14px",marginBottom:8}}>
            ...
            {(()=>{const st_=semit(s.key,item.key);return st_!==0&&<div style={{marginTop:6,fontSize:11,color:"var(--ind)",fontWeight:600}}>↑ Transposé de {st_} demi-ton(s) depuis {s.key}</div>;})()}
          </div>
        ):null;})}
      </div>
      ...
      <div ref={printRef}>
        <div style={{display:"flex",alignItems:"center",gap:14,borderBottom:"2px solid var(--bdr)",paddingBottom:14,marginBottom:18}}>
          ...
          <div>
            ...
            <div style={{fontSize:12,color:"var(--txt2)"}}>{program.date&&new Date(program.date+"T00:00:00").toLocaleDateString("fr-FR",{weekday:"long",day:"numeric",month:"long",year:"numeric"})} — {CHURCHES[program.churchId]?.fullName||"Église de Creil"}{program.notes&&` · ${program.notes}`}</div>
          </div>
        </div>
